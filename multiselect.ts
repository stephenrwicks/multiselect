class MultiSelect extends HTMLElement {
    ...
    #map: Map<HTMLOptionElement, HTMLInputElement> = new Map();
    #groupMap: Map<HTMLOptGroupElement, HTMLFieldSetElement> = new Map();
    ...
    attributeChangedCallback(name: string, oldValue: any, newValue: any) {
        ...
        if (name === 'disabled') {
            ...
            if (this.#required) this.ariaRequired = 'true';
    ...
    #dismiss = (e: Event) => {
        ...
        if (e.target === this.#optionContainer) return;
        if (this.#optionContainer.contains(e.target as Node)) return;
        this.hide();
    };
    ...
    #buildOption(option: HTMLOptionElement) {
        const label = document.createElement('label');
    ...
    #search(searchText = '') {
        ...
        for (const [option, checkbox] of this.#map) {
            if (!searchText) {
                (checkbox.parentElement as HTMLLabelElement).style.display = '';
                continue;
            }
    ...
    get type(): 'select-multiple' | 'select-one' {
        if (this.multiple) return 'select-multiple';
        return 'select-one';
    }
    ...
    add(option: HTMLOptionElement | HTMLOptGroupElement, before?: HTMLElement | number | null): void {
        if (!(option instanceof HTMLOptionElement) && !(option instanceof HTMLOptGroupElement)) return;
        this.#native.add(option, before);
        // "before" is not yet supported
        if (option instanceof HTMLOptionElement) this.#optionContainer.append(this.#buildOption(option));
        else if (option instanceof HTMLOptGroupElement) this.#optionContainer.append(this.#buildGroup(option));
        this.#updateSelectAllCheckbox();
        this.#updateDisplay();
        this.#updateValidity();
        this.#updateFormData();
    ...
    remove(index?: number): void {
        if (typeof index === 'undefined') super.remove();
        if (typeof index !== 'number') return;
        if (index > this.#native.options.length) return;
    ...
    set showSearch(val) {
        this.#showSearch = !!val;
        if (this.#showSearch) {
            if (this.#showSelectAll) this.#selectAllOption.before(this.#searchContainer);
    ...
    set showSelectAll(val) {
        ...
        if (this.#showSelectAll) {
            if (this.#showSearch) this.#searchContainer.after(this.#selectAllOption);
            else this.#optionContainer.prepend(this.#selectAllOption);
        }
        else this.#selectAllOption.remove();
    }

    // Returns options not hidden by search
    get visibleOptions(): Array<HTMLOptionElement> {
        return [...this.options].filter(o => {
            const checkbox = this.#map.get(o);
            if (!checkbox) return false;
            if (checkbox.parentElement?.style?.display === 'none') return false;
            return true;
        });
    }
